...
int processCreateResponse(Message *response)
{
    ...
    request1.datasize = snprintf(request1.data, DATA_SIZE, "CREATE %d", fileNo);
    request2.datasize = snprintf(request2.data, DATA_SIZE, "CREATE %d", fileNo);
    request3.datasize = snprintf(request3.data, DATA_SIZE, "CREATE %d", fileNo);
